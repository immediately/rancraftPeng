...
public class EntityPenguinCS extends EntityPenguin
{
    final int MAXHEALTHWILD = 6;
    final int MAXHEALTHTAME = 13;
    ...
    public EntityPenguinCS(World par1World)
    {
        super(par1World);
        //texture = texturePath + "pengCS.png";
    ...
    protected String getLivingSound()
    {
        if (isAngry())
        {
            return RanCraftPenguins.modID+":penguinCS.angry";
        }

        if (rand.nextInt(RanCraftPenguins.pengQuietInt) == 0) {
	        if(dataWatcher.getWatchableObjectInt(18) < healthTercile1) {
	    		return RanCraftPenguins.modID+":penguinCS.healthd";
	    	} else if(dataWatcher.getWatchableObjectInt(18) < healthTercile2) {
	    		return RanCraftPenguins.modID+":penguinCS.healthc";
	    	} else if(dataWatcher.getWatchableObjectInt(18) < healthTercile3) {
	    		return RanCraftPenguins.modID+":penguinCS.healthb";
	    	} else {
	    		return RanCraftPenguins.modID+":penguinCS.healtha";
	    	}
        } else {
        	return null;
        }
    ...
    {
        return RanCraftPenguins.modID+":penguinCS.hurt";
    }
    ...
    {
        return RanCraftPenguins.modID+":penguinCS.death";
    }
    ...
    public Item droppedFeather()
    {
		...
		if(i > 0){ // 66% black striped, 33% white
     ...
     */
    public EntityPenguinCS spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityPenguinCS var2 = new EntityPenguinCS(this.worldObj);
    ...
    public boolean canMateWith(EntityAnimal par1EntityAnimal)
    {
        ...
        else if (!(par1EntityAnimal instanceof EntityPenguinCS))
        {
            return false;
        }
        else
        {
            EntityPenguinCS var2 = (EntityPenguinCS)par1EntityAnimal;
