...
public class EntityPenguinCloud extends EntityPenguin {
	...
	final int MAXHEALTHTAME = 17; // not used currently, b/c they're not tameable
	...
	protected int getDropItemId() {
		return RanCraftPenguins.PengSkinLightBlue.itemID;
	}
	...
	public boolean interactShearing(EntityPlayer par1EntityPlayer) {
		if (!worldObj.isRemote) {
			...
			if (itemstack != null) { // make sure we're holding something first to prevent crash
		        if ((itemstack.itemID == Item.shears.itemID) || (itemstack.itemID == RanCraftPenguins.PenguinShears.itemID)){
					// shear
					if (isDisguised(par1EntityPlayer)) {
			        	...
			        	{
			        		setAngry(true);
			        		if (itemstack.itemID == RanCraftPenguins.PenguinShears.itemID)
			        		{
    ...
    protected boolean isDisguised(EntityPlayer entityplayer1)
    {
		...
		//System.out.printf("%b, %b, %b\n", inventoryplayer1.armorItemInSlot(0).itemID, RanCraftPenguins.PenguinBootsCloudID, RanCraftPenguins.PenguinFlameFlippersID);
