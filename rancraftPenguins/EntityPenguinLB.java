...
public class EntityPenguinLB extends EntityPenguin
{
    final int MAXHEALTHWILD = 5;
    final int MAXHEALTHTAME = 12;
    ...
    public EntityPenguinLB(World par1World)
    {
        super(par1World);
    ...
    protected String getLivingSound()
    {
        if (isAngry())
        {
            return RanCraftPenguins.modID+":penguinLB.angry";
        }

        if (rand.nextInt(RanCraftPenguins.pengQuietInt) == 0) {
	        if(dataWatcher.getWatchableObjectInt(18) < healthTercile1) {
	    		return RanCraftPenguins.modID+":penguinLB.healthd";
	    	} else if(dataWatcher.getWatchableObjectInt(18) < healthTercile2) {
	    		return RanCraftPenguins.modID+":penguinLB.healthc";
	    	} else if(dataWatcher.getWatchableObjectInt(18) < healthTercile3) {
	    		return RanCraftPenguins.modID+":penguinLB.healthb";
	    	} else {
	    		return RanCraftPenguins.modID+":penguinLB.healtha";
	    	}
        } else {
        	return null;
        }
    ...
    {
        return RanCraftPenguins.modID+":penguinLB.hurt";
    }
    ...
    {
        return RanCraftPenguins.modID+":penguinLB.death";
    }
    ...
    {
		return RanCraftPenguins.PengSkinBlue.itemID;
    }
    ...
    public Item droppedFeather()
    {
		...
		if(i < 3){ // 30% white, 70% blue
			dropped = RanCraftPenguins.PengFeatherWhite;
		} else {
			dropped = RanCraftPenguins.PengFeatherBlue;
		}
		return dropped;
    }
     ...
     */
    public EntityPenguinLB spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityPenguinLB var2 = new EntityPenguinLB(this.worldObj);
    ...
    public boolean canMateWith(EntityAnimal par1EntityAnimal)
    {
        ...
        else if (!(par1EntityAnimal instanceof EntityPenguinLB))
        {
            return false;
        }
        else
        {
            EntityPenguinLB var2 = (EntityPenguinLB)par1EntityAnimal;
