...
public class EntityPenguinMag extends EntityPenguin
{
    final int MAXHEALTHWILD = 7;
    final int MAXHEALTHTAME = 17;
    ...
    public EntityPenguinMag(World par1World)
    {
        super(par1World);
    ...
    /*public String getTexture()
    {
        if (isChild()){
        	if (isTamed())
                return texturePath + "pengMagCh_tame.png";
            else
            	return texturePath + "pengMagCh.png";
        } 
        if (isAngry()) {
           	return texturePath + "pengMag_angry.png";
        } 
       	if (isTamed()) {
            return texturePath + "pengMag_tame.png";
    	}
    ...
    protected String getLivingSound()
    {
        if (isAngry())
        {
            return RanCraftPenguins.modID+":penguinMag.angry";
        }

        if (rand.nextInt(RanCraftPenguins.pengQuietInt) == 0) {
	        if(dataWatcher.getWatchableObjectInt(18) < healthTercile1) {
	    		return RanCraftPenguins.modID+":penguinMag.healthd";
	    	} else if(dataWatcher.getWatchableObjectInt(18) < healthTercile2) {
	    		return RanCraftPenguins.modID+":penguinMag.healthc";
	    	} else if(dataWatcher.getWatchableObjectInt(18) < healthTercile3) {
	    		return RanCraftPenguins.modID+":penguinMag.healthb";
	    	} else {
	    		return RanCraftPenguins.modID+":penguinMag.healtha";
	    	}
        } else {
        	return null;
        }
    ...
    {
        return RanCraftPenguins.modID+":penguinMag.hurt";
    }
    ...
    {
        return RanCraftPenguins.modID+":penguinMag.death";
    }
    ...
    */
   public EntityPenguinMag spawnBabyAnimal(EntityAgeable par1EntityAgeable)
   {
       EntityPenguinMag var2 = new EntityPenguinMag(this.worldObj);
   ...
   public boolean canMateWith(EntityAnimal par1EntityAnimal)
   {
       ...
       else if (!(par1EntityAnimal instanceof EntityPenguinMag))
       {
           return false;
       }
       else
       {
           EntityPenguinMag var2 = (EntityPenguinMag)par1EntityAnimal;
